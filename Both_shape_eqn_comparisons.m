...
epsilon = -1;
n0 = 1;
% Sigma = 3.815827685907020e+04;
Sigma = 1;
kappa = 1;
lambda = sqrt(kappa/Sigma);

% get the shape of the free region
% phi_vals = deg2rad(linspace(70, 120, 5));
% h0_vals = linspace(-1.5,0.5,20);
% phi_vals = deg2rad(95);
phi_vals = 3.054326190990077;
h_phi_vals = -R/2;
h_phi_init = -0.45;
psi_dot_init = 1;
...
%     psi_dot_init_final = -1;
    psi_dot_init_final = -sqrt(2*(1-cos(phi)))/lambda*5
    ...
    r_nonlin = solution(2,:);
    h_nonlin = solution(3,:)-R*sin(phi);
    
    psi_end = out.y(1,end);
    diffs = psi_end^2;
    diffs = (r_nonlin(end)-d/2).^2
    out.xe
    %%%%%%
