function fig_handle = delta_E_abs_value_combined(xaxis_name, xscale, yaxis_name, yscale,...
...
E_combined(1,:) = E_all(1,:);
E_combined(2,:) = E_all(2,:);
E_combined(3,:) = E_all(3,:)+E_all(4,:);
E_combined(4,:) = E_all(5,:)+E_all(6,:);
for ii=1:4
%     plot(param_1_vals, E_all(ii,:)./(vs.kD.*vs.d_vals.^2), ...
%         strcat(colours(ii),lines(ii)))
    plot(param_1_vals, abs(E_combined(ii,:)), ...
        strcat(colours(ii),lines(ii)))
end
...
legend({'$E_\mathrm{total}$','$E_\mathrm{adhesion}$',...
    '$E_\mathrm{stretch}$','$E_\mathrm{bend}$'}, 'Box','off',...
    'location', 'best')
