function fig_handle = E_absolute_value(xaxis_name, xscale, yaxis_name, yscale,...
...
colours_cb = ["#27E0D8", "#648FFF", "#1D0C6B", "#DC267F", "#FE6100", ...
    "#FFB000", "#016D24"];
for ii=1:6
    plot(param_1_vals, abs(E_all(ii,:)), ...
        strcat(colours(ii),lines(ii)))
end
min_E = min(abs(E_all), [], 'all')*1.1+eps(1);
max_E = max(abs(E_all), [], 'all')*1.1;
